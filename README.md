...
- Pillow — обработка загрузок изображений (аватары, картинки к постам, фото в личных сообщениях).
...
- Дополнительно понадобятся:
  ...
  - Компилятор C/библиотеки для сборки Pillow (обычно уже есть в стандартной системе разработки).
...
- Вынесение чувствительных параметров (секретный ключ, параметры БД) в переменные окружения и отдельные конфигурационные файлы.
